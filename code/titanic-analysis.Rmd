...
Per a aquest estudi, utilitzarem les files que provenen del joc de dades "train.csv" i farem una comparació entre la població de persones que va sobreviure i les que no (**Survived**) per veure com va poder influenciar la variable edat en aquests fets (**Age**).
...
train_dataset <- dataset[dataset$row_type == "train", ]
survived_dataset <- train_dataset[train_dataset$Survived == 1, ]
did_not_survive_dataset <- train_dataset[train_dataset$Survived == 0, ]
...
set.seed(1) 
result <- var.test(Age ~ Survived, data = train_dataset)
print(result)
...
test_dataset <- rbind(survived_dataset[1:min_rows, ], did_not_survive_dataset[1:min_rows, ])

# Realitzem el t-test
T_test <- t.test(Age ~ Survived, data = test_dataset, paired = TRUE)
T_test

# Il·lustrem el t-test
ggplot(train_dataset, aes(as.factor(Survived), Age)) + geom_boxplot()
...
ANOVA_test <- aov(Age ~ Survived, data = train_dataset)
summary(ANOVA_test)
...
model <- glm(Survived ~ Age + Sex + Pclass + Embarked,family=binomial(link = 'logit'), data=train_dataset)
summary(model)
...
- Després d'aplicar i executar la prova t-test en les mostres de població podem concloure que, amb un 95% de nivell de confiança i un p-valor de 0.6741, no existeix    diferència entre les mitjanes de les poblacions, d'aquesta manera provem que l'edat no va ser una variable rellevent en quant a la supervivència del Titanic.         Aquesta mateixa conclusió també la corroborem amb la prova ANOVA (one-way analysis of variance) ja que el p-valor obté un resultat de 0.505, clarament superior a     0.05.
- El model de regressió logístic lineal creat per predir la probabilitat que les diverses variables siguin significatives en quan a la supervivencia del naufragi, ens   expliquen que el fet de ser home o dona i de pertanya a una classe o a una altra va ser clarament significatiu en relació a la supervivència del Titanic ja que el    pvalor de Sex i Pclass són superior a 2 ja sigui en positiu o negatiu.
-
-
-
